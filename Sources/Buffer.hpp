...
class BufferLayout 
{
    public:
        ...
        const void SetVertexBufferDesc(Vertex_Buffer_Desc* desc) const 
        {   
            desc->bindingDesc.binding = 0;
            desc->bindingDesc.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
            desc->bindingDesc.stride = mStride;
            ...
            for(int i = 0; i < mAttribs.size(); i++) 
            {
                ...
                desc->attribsDesc.push_back(attribDesc);
            }
        }
