...
public:
     ...
     * @param position x, y and z coordiantes of the PerspectiveCamera's position in world space.
     */
    explicit PerspectiveCamera(glm::vec3 position, glm::vec3 target = {0.0f, 0.0f, 0.0f}) 
     ...
     */
    [[nodiscard]] constexpr const glm::mat4& GetProjectionMatrix(const uint32_t width, const uint32_t height) const noexcept 
    {
        return glm::perspective(glm::radians(mZoom), (float)width / height, mNear, mFar);
    }
